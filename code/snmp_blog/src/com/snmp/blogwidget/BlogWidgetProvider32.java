...
public class BlogWidgetProvider32 extends AppWidgetProvider {
    ...
    public static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int updateAppWidget) {
        RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.appwidget_provider_blog);

        remoteViews.setTextViewText(R.id.appwidget_text_blog, SelectDialog.getSelectPref());
        LogUtils.i(TAG, "updateAppWatchWidget bbbbbb6 " + SelectDialog.getSelectPref());

        remoteViews.setOnClickPendingIntent(R.id.appwidget_blog_view,
                PendingIntent.getActivity(context, 0, new Intent(context, BlogListActivity.class), 201326592));
